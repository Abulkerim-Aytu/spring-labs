...
public class CartServiceImpl implements CartService {
    ...
    public void initialiseCartList() {
        ...
        cart1TotalAmount = PRODUCT_LIST.stream()
                .map(product -> product.getPrice().multiply(BigDecimal.valueOf(product.getQuantity())))
                .reduce(BigDecimal.ZERO, BigDecimal::add);;
//        for (CartItem cartItem : cartItemList){
//            cart1TotalAmount =  art1TotalAmount.add(cartItem.getProduct().getPrice().multiply(BigDecimal.valueOf(cartItem.getQuantity())));
//        }
        ...
        // todo change to stream
        cart2TotalAmount = PRODUCT_LIST.stream()
                .map(product -> product.getPrice().multiply(BigDecimal.valueOf(product.getQuantity())))
                .reduce(BigDecimal.ZERO, BigDecimal::add);;
